...
def create_crew_for_movie(sender, instance, created, **kwargs):
    if created:
        # Loop over the directors and create Crew objects for each if they
        # do not already exist
        for director in instance.directors.all():
            crew, created = (
                Crew.objects.get_or_create(crew_name=director.crew_name)
            )
            if created:
        ...
        # Loop over the main cast and create Crew objects for each if they
        # do not already exist
        for actor in instance.main_cast.all():
            crew, created = Crew.objects.get_or_create(
                crew_name=actor.crew_name
            )
            if created:
                instance.main_cast.add(crew)

        # Save the instance again to create the main cast Crew objects 
        # in the database
        instance.save()
